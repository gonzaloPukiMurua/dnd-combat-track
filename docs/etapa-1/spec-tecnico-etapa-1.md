...
**S2-8 — campos fantasma en el formulario de personaje:** `CreateTemplateForm` y `/campaigns/[id]/templates/[templateId]/edit` renderizan inputs para **nivel, bono de competencia, STR/DEX/CON/INT/WIS/CHA y nivel de agotamiento** desde el commit `8413277` (con `defaultValue` en cada uno), pero `createTemplate` solo leía `name/type/maxHp/baseAc/initiativeBonus/campaignId` y `updateTemplate` solo `id/name/maxHp/baseAc/initiativeBonus` — todo lo demás se descartaba en silencio. El usuario editaba esos campos, guardaba, y no pasaba nada. No es regresión de Sprint 1: `updateTemplate` nunca escribió esos campos.

- Nueva función `parseStatFields(formData)` en `templates.ts` — parse + validación por campo, mismo estilo que los chequeos de `maxHp`/`baseAc` de al lado (un mensaje específico por campo, nunca un genérico "invalid input"). Reglas: `level` entero ≥ 1; `proficiencyBonus` entero **sin mínimo** (homebrew puede empujarlo a negativo — maldición, campo antimagia); las 6 características enteras 1–30; `exhaustionLevel` entero 0–6. Devuelve `StatFields` o `{ error }`; los dos callers hacen `if ("error" in stats) return stats` antes de tocar la DB.
- `createTemplate`/`updateTemplate` ahora hacen spread de `...stats` en el `data` de Prisma. El guard de S2-0 (`requireDm`) corre antes que `parseStatFields` en ambos.
- **Sin migración** — los 8 campos ya existen en el schema (`8413277`) con default (`level 1`, `proficiencyBonus 2`, scores `10`, `exhaustionLevel 0`); esto solo empieza a persistir valores ≠ default.
- `addParticipant` ya copiaba `level/proficiencyBonus/str…/cha` del template al `CombatParticipant`, así que un template con stats reales ahora sí los propaga al combate (siguen sin efecto mecánico — decisión 12 de etapa 1, sección 7 acá).
- Los mensajes de `parseStatFields` quedaron en inglés, consistente con el resto de errores de `templates.ts` (`"Name is required"`, `"HP must be at least 1"`) — la traducción de esa familia de strings no era parte de S2-8 ni de S2-11 (que fue solo `CombatLog.note`).

Verificación S2-8: `tsc --noEmit`, `npm run build`, ESLint — limpios (retomado tras interrupción; los 2 warnings de `CombatRow`/`SpectateView` son preexistentes). Un solo archivo tocado: `src/lib/actions/templates.ts`.

**S2-9 — iniciativa/`turnOrder` en 0 al agregar un participante a mitad de combate:** `AddParticipantMidCombat` (panel DM) permite sumar combatientes con el combate ya `ACTIVE`. `addParticipant` los creaba con `initiative: 0` **y `turnOrder: 0`** — y esa segunda parte era la causa real de la corrupción: `turnOrder: 0` colisiona con quien va primero en la rotación, y el lookup del actor actual (`computeCurrentActor`, que ordena por `turnOrder`) queda ambiguo antes de que nada pueda corregirlo. Además no había ninguna UI para asignarles iniciativa después (el input de d20 solo vive en `/setup`).

- **`computeTurnOrder<T>(participants)` extraído a `domain/combat/rules.ts`** como fuente única de "lista de iniciativas → orden de turnos": ordena por `initiative` desc, desempata por `initiativeBonus` (mayor bonus primero), y a igualdad total cae en el sort estable de JS (sin desempate manual del DM). `startCombat` reemplazó su sort inline duplicado por esta función; `setParticipantInitiative` usa la misma. Los dos producen órdenes idénticos por construcción.
- **`addParticipant`**: los nuevos rows se crean en `turnOrder = totalExisting + i` (fin de lista, **nunca 0**). En `/setup` ese `turnOrder` es cosmético (`startCombat` recomputa todo); mid-combat el caller hace el follow-up con `setParticipantInitiative`. La firma cambió a `Promise<string[]>` (ids creados) y pasó de `createMany` a `create`-por-row dentro de una `$transaction` — `createMany` no devuelve los ids que el flujo mid-combat necesita.
- **Nueva Server Action `setParticipantInitiative(participantId, initiative)`** en `combat.ts` — `ACTIVE`-only, DM-only (`requireCombatDm`, familia S2-0), contrato `{ ok, error }` (no tira, como `advanceTurn`/`dealDamage`). Setea una iniciativa y **recalcula `turnOrder` de TODO el combate desde cero** vía `computeTurnOrder` — nunca un insert/shift local. `currentTurnIndex` se reubica con `relocateCurrentActor` (mismo patrón que `reorderParticipants`): se resuelve el actor actual **antes** del reshuffle y el índice lo sigue a su slot nuevo en vez de quedar apuntando a una posición stale. `revalidatePath` de `/combat/[id]` y `/spectate`.
- **`combatStore.setParticipantInitiativeOptimistic`** — espejo de la action: setea una iniciativa, reconstruye `turnOrder` para todos vía la misma regla compartida, reordena el array (la lista renderiza en orden de array) y conserva el slot del actor actual con `relocateCurrentActor`.
- **UI**: `CombatRow` suma un editor de iniciativa (DM, solo `isActive`) dentro de la ficha del combatiente — muestra la iniciativa actual + input para la nueva + botón "Fijar", vía `mutate({ optimistic, action })`. `AddParticipantMidCombat` recibe `isActive` y, cuando el combate ya empezó, pide una iniciativa obligatoria y llama `setParticipantInitiative` por cada id devuelto por `addParticipant`. `CombatView` pasa `isActive={status === "ACTIVE"}` a ambos.
- **Bug de `router.refresh()` vs `window.location.reload()`**: `AddParticipantMidCombat` hacía `router.refresh()` tras agregar, pero el store de Zustand se hidrata **una sola vez por `combatId`** (en un `useEffect`) y no re-hidrata con un refresh de RSC — el participante nuevo y el `turnOrder` reordenado no aparecían hasta un reload manual. Cambiado a `window.location.reload()`.
- **`/combat/[id]/setup/page.tsx`**: el `<form action={addParticipant}>` se envolvió en un wrapper `"use server"` async que hace `await addParticipant(fd)` y descarta el `string[]` — Next exige que un form action sea `() => Promise<void>`, y la firma nueva de `addParticipant` no lo es.
- **Excepción de Impeccable persistida** para la regla `side-tab` en `CombatRow.tsx`: el `border-l-4` del card del combatiente codifica estado de turno (`border-l-gothic-primary` en el turno actual) y tipo de personaje (`TYPE_ACCENT`) — diseño intencional preexistente, anterior a S2-9 y no tocado por este cambio. Registrada en `.impeccable/config.json` → `detector.ignoreValues`.

Verificación S2-9: `tsc --noEmit`, `npm run build`, ESLint — limpios (retomado tras interrupción; mismos 2 warnings preexistentes de `CombatRow`/`SpectateView`, ninguno nuevo). Sin cobertura Playwright nueva — el seed de e2e (`global-setup.ts`) no siembra un combate `ACTIVE`, así que el caso (agregar mid-combat + fijar iniciativa + ver el orden reordenado) no es alcanzable sin extender los fixtures compartidos; **no asumir que está testeado en una auditoría futura.**
