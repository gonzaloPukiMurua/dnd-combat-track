...
- Nuevo módulo `src/lib/auth/action-guards.ts` — contraparte de escritura de `guards.ts`: tira `UnauthorizedError("No autorizado")` en vez de `notFound()`, porque una acción no tiene página a la que devolver un 404. Exporta `requireCampaignMembership`/`requireCampaignDmAction` (nivel campaña), `requireCombatMembership`/`requireCombatDm` (resuelve la campaña dueña de un combate vía `Combat.campaignId`), `requireParticipantAccess` (DM: cualquier participante de su campaña; Jugador: solo el participante cuyo template tiene `ownerId === userId` — la misma regla `isMyCharacter` que `SpectateView` ya aplicaba solo del lado del cliente) y `requireParticipantDmAccess` (controles DM-only que comparten componente con el jugador: PV temporales, modificadores de AC, toggles de acción). Los guards de combate devuelven `{ campaignId, membership }` y los de participante `{ participantId, combatId, campaignId, membership }` ya resueltos, para que la acción reutilice esos valores sin re-consultar (p. ej. `dealDamage` toma de ahí el `combatId` que escribe en el `CombatLog`).
...
- `templates.ts`/`groups.ts` (form-state `{ error }`): un adaptador local `requireDm()` en cada uno (5 funciones en `templates.ts`, 4 en `groups.ts`); la campaña sale del `formData` en las de creación y del registro del template/grupo en el resto. `longRest`/`shortRest` ahora exigen que todos los ids existan y resuelvan a una única campaña antes de tocar nada. `removeGroupMember` chequea que el miembro pertenezca a `groupId`.
...
- Casos borde al cerrar (contra código real): (1) el frontend (`SpectateView`, `CombatRow`, `CurrentTurnPanel`, `DeathSaveTracker`) sigue mandando `combatId` en el `formData` de las 5 acciones con bitácora — no es bug: server-side ese valor quedó solo como chequeo de presencia y el `CombatLog.combatId` real sale del participante verificado (`resetDeathSaves` lo ignora del todo). No se tocó el cliente para sacarlo (obligaría a editar `makeFormData` en 4 componentes por un no-op). (2) Ninguna firma de Server Action cambió y `participant.ts` mantiene su contrato `{ ok } | { ok: false, error }`: `useCombatMutation` ya rutea `{ ok: false }` a toast + rollback, así que `"No autorizado"` fluye sin tocar componentes. `SpectateView` y `AddParticipantMidCombat` ignoran el retorno (patrón preexistente) — un fallo de auth ahí es no-op silencioso, pero inalcanzable en uso normal por el gating de rol que el cliente ya hace (`isMyCharacter`, panel DM). (3) `"No autorizado"` se renderiza como string plano en caja de error en `TemplateCard`/`GroupCard`/`RestPanel`/`CreateTemplateForm`/`groups/new`; los server actions inline de la página de edición de template y del detalle de grupo se tragan cualquier `result.error` (preexistente), pero esas rutas son DM-only por `layout.tsx` + `page.tsx`.
